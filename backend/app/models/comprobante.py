...
class Comprobante(Base):
    ...
    __table_args__ = (
        ...
        # Un comprobante solo puede ser reversado una vez: la unicidad la garantiza la BD,
        # no solo la validación de servicio.
        UniqueConstraint("reversa_comprobante_id", name="uq_comprobante_reversa_unica"),
    )
    ...
    # Solo lo lleva el comprobante espejo, apuntando al original que anula.
    reversa_comprobante_id: Mapped[int | None] = mapped_column(
    ...
    reversa_de: Mapped[Comprobante | None] = relationship(
        remote_side="Comprobante.id",
        foreign_keys=[reversa_comprobante_id],
    )
...
class LineaContable(Base):
    ...
    tercero_id: Mapped[int | None] = mapped_column(ForeignKey("terceros.id"), nullable=True)
